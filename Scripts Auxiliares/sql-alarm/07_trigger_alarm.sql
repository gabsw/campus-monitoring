...
BEGIN
	...
	IF (NEW.temperature > max_temp AND (status_max_temp_sensor IS NULL OR status_max_temp_sensor = FALSE)) THEN
		INSERT INTO campus_monitoring.UNIVERSAL_ALARM(date_time, sensor_id,violation_type, violation_parameter, violation_value, status)
       	VALUES(NEW.date_time, NEW.sensor_id, 'max', 'temperature', NEW.temperature, TRUE);
	END IF;
		
	IF (NEW.temperature < min_temp AND (status_min_temp_sensor IS NULL OR status_min_temp_sensor = FALSE)) THEN
		INSERT INTO campus_monitoring.UNIVERSAL_ALARM(date_time, sensor_id,violation_type, violation_parameter, violation_value, status)
       	VALUES(NEW.date_time, NEW.sensor_id, 'min', 'temperature', NEW.temperature, TRUE);
	END IF;
		
	IF (NEW.humidity > max_hum AND (status_max_hum_sensor IS NULL OR status_max_hum_sensor = FALSE)) THEN
		INSERT INTO campus_monitoring.UNIVERSAL_ALARM(date_time, sensor_id,violation_type, violation_parameter, violation_value, status)
       	VALUES(NEW.date_time, NEW.sensor_id, 'max', 'humidity', NEW.humidity, TRUE);
	END IF;
		
	IF (NEW.humidity < min_hum AND (status_min_hum_sensor IS NULL OR status_min_hum_sensor = FALSE)) THEN
		INSERT INTO campus_monitoring.UNIVERSAL_ALARM(date_time, sensor_id,violation_type, violation_parameter, violation_value, status)
       	VALUES(NEW.date_time, NEW.sensor_id, 'min', 'humidity', NEW.humidity, TRUE);
	END IF;
		
	IF (NEW.co2 > max_co2 AND (status_max_co2_sensor IS NULL OR status_max_co2_sensor = FALSE)) THEN
		INSERT INTO campus_monitoring.UNIVERSAL_ALARM(date_time, sensor_id,violation_type, violation_parameter, violation_value, status)
       	VALUES(NEW.date_time, NEW.sensor_id, 'max', 'co2', NEW.co2, TRUE);
	END IF;
