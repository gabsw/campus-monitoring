...
public interface WeatherRepository extends JpaRepository<WeatherReading, WeatherReadingPK> {
	...
	@Query(value = "Select temperature, humidity, co2, date_time, sensor_id, local_name from campus_monitoring.WEATHER_READING join campus_monitoring.SENSOR on campus_monitoring.WEATHER_READING.sensor_id=campus_monitoring.SENSOR.id where date_time>=?1 and date_time<=?2 order by date_time asc;", 
			  nativeQuery = true)
	...
	@Query(value = "Select temperature, humidity, co2, date_time, sensor_id, local_name from campus_monitoring.WEATHER_READING join campus_monitoring.SENSOR on campus_monitoring.WEATHER_READING.sensor_id=campus_monitoring.SENSOR.id where local_name=?1 order by date_time asc;", 
			  nativeQuery = true)
	List<WeatherReading> findByLocalNameOrderByWeatherReadingPKDateTimeAsc(String local);

	@Query(value = "Select temperature, humidity, co2, date_time, sensor_id, local_name from campus_monitoring.WEATHER_READING join campus_monitoring.SENSOR on campus_monitoring.WEATHER_READING.sensor_id=campus_monitoring.SENSOR.id where local_name=?1 and date_time>=?2 and date_time<=?3 order by date_time asc;", 
			  nativeQuery = true)
